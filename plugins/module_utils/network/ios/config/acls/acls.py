...
class Acls(ResourceModule):
        ...
        if self.state == "deleted":
            for k, v in iteritems(haved):
                ...
                if (afi_want and not afi_want.get("acls")) or not wantd:
                    ...
                    for key, val in iteritems(v["acls"]):
                        if want_acl := afi_want["acls"].get(key):
                            val.update({"afi": afi, "name": key})
                            self.addcmd(val, "acls_name", True)
    ...
    def _compare(self, want, have):
        ...
        if want == have or self.state == "deleted":
            return
        if want.get("acls"):
            afi = want["afi"]
                    ...
                    if have_acl:
                        for key, val in iteritems(v.get("aces")):
                            ...
                            if have_ace and val != have_ace:
                                ...
                                self.compare(parsers=self.parsers, want={"aces": val, "afi": afi}, have={})
                    if self.state in ["overridden", "replaced"] and have_acl.get(
                        "aces"
                    ):
                        for key, val in iteritems(have_acl["aces"]):
                            self.compare(parsers=self.parsers, want={}, have={"aces": val, "afi": afi})
    ...
    def update_sequence_in_want(self, want, have):
        if (want and not have) or (not want and have):
            return want
        temp_acl = {}
        for count, each in enumerate(want):
            if each.get("acls"):
    ...
    def _rearrange_replace_overridden_config_cmd(self, commands):
        ...
        temp_acl_config = []
        ace_config = False
        for acl, cmd in enumerate(commands):
            if "no ip access-list" in cmd or "no ipv6 access-list" in cmd:
                ...
                ace = len(temp_acl_config) - 1 if ace_config else len(temp_acl_config)
        ...
        for each in param:
            ...
            if each.get("acls"):
                ...
                for every in acls:
                    ...
                    temp_aces = {}
                    if aces := every.get("aces"):
                        for count, ace in enumerate(aces):
                            seq = (
                                ace["sequence"]
                                if ace.get("sequence")
                                else count
                            )
                            temp_aces[f"{acl_name}_{str(seq)}"] = ace
                    if every.get("acl_type"):
                        ...
                        temp_acls[acl_name] = {"aces": temp_aces}
            each["acls"] = temp_acls
            temp[each["afi"]] = {"acls": temp_acls}
        return temp
